...
const Admin = () => {
  ...
  return (
    <div className='flex min-h-screen'>
      {/* Sidebar */}
      <aside
        className={`w-64 bg-white text-black flex flex-col fixed top-0 left-0 h-full z-50 transform ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 transition-transform duration-200 ease-in-out p-4`}
      >
        ...
        <button
          className='flex items-center p-4 pl-6 hover:bg-[#d9f2ff] hover:text-[#0087d0]'
          onClick={() => handleMenuClick("CrossTransplant")}
        >
      ...
      {/* Mobile Menu Button */}
      <button
        className='md:hidden w-10 h-10 fixed top-3 left-1 bg-white flex items-center justify-center z-50'
      ...
      {/* Main Content */}
      <main className="flex-1 bg-gray-100 p-2 md:p-6 md:ml-64">
        {renderComponent()}
      </main>
    </div>
  );
